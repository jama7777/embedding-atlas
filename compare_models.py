...
for m in built:
    k = m["key"]
    if m.get("loader") == "api":     # API model: its "vocab" is the embedded word list
        words = json.load(open(f"{CACHE}/{k}_words.json"))
        full_vocab[k] = {normalize(w) for w in words}
    else:
        tok = AutoTokenizer.from_pretrained(m["name"], token=HF_TOKEN, trust_remote_code=True)
        full_vocab[k] = {normalize(t) for t in tok.get_vocab().keys()}
...
fig.write_html("compare.html", include_plotlyjs=True)
print("\nSaved -> compare.html")
...
f2.tight_layout(); f2.savefig("compare_matrices.png", dpi=130)
print("Saved -> compare_matrices.png")
...
A.selectedIndex = 0; B.selectedIndex = Math.min(2, D.models.length - 1);
...
with open("compare_ui.html", "w") as f:
    f.write(UI.format(data=json.dumps(UI_DATA)))
print("Saved -> compare_ui.html")
