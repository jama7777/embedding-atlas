...
MODELS = [
    ...
    {"key": "gemma",    "loader": "llm", "name": "google/gemma-2-2b", "full": True,
    ...
    # --- standard encoder models (SentenceTransformers) ---
    {"key": "bge",      "loader": "st",  "name": "BAAI/bge-base-en-v1.5",
     "label": "BGE-base-en-v1.5 · 768d · WordPiece · 30k"},
    {"key": "e5",       "loader": "st",  "name": "intfloat/e5-base-v2",
     "label": "E5-base-v2 · 768d · WordPiece · 30k"},
    {"key": "nomic",    "loader": "st",  "name": "nomic-ai/nomic-embed-text-v1",
     "label": "Nomic-embed-text-v1 · 768d · WordPiece · 30k"},
    {"key": "gte",      "loader": "st",  "name": "Alibaba-NLP/gte-base-en-v1.5",
     "label": "GTE-base-en-v1.5 · 768d · WordPiece · 30k"},
    {"key": "gist",     "loader": "st",  "name": "avsolatorio/GIST-Embedding-v0",
     "label": "GIST-Embedding-v0 · 768d · WordPiece · 30k"},
    # --- static word similarity models ---
    {"key": "glove",    "loader": "st",  "name": "sentence-transformers/average_word_embeddings_glove.6B.300d", "full": True,
     "label": "GloVe-6B-300d FULL · 300d · Static Word Embeddings · 400k"},
    {"key": "komninos", "loader": "st",  "name": "sentence-transformers/average_word_embeddings_komninos", "full": True,
     "label": "Komninos-100d FULL · 100d · Static Word Embeddings · 222k"},
    # --- random baseline experiment ---
    {"key": "swaram-li1", "loader": "random", "name": "swaram-li1", "full": True,
     "label": "Swaram-li1 FULL · 2084d · Random Vector Baseline · 400k"},
]
...
def build_model(m):
    ...
    if key == "swaram-li1":
        # Load GloVe to get the identical 400,001 word tokens
        print(f"[{key}] Generating random vectors for the 400,001 GloVe vocabulary...")
        from sentence_transformers import SentenceTransformer
        glove_model = SentenceTransformer("sentence-transformers/average_word_embeddings_glove.6B.300d")
        tok = list(glove_model[0].tokenizer.vocab)
        
        # Generate random vectors in the exact GloVe initialization range [-0.5 / d, 0.5 / d]
        dim = 2084
        rng = np.random.default_rng(42)
        emb = rng.uniform(low=-0.5 / dim, high=0.5 / dim, size=(len(tok), dim)).astype(np.float32)
        print(f"[{key}] Generated GloVe-style raw random matrix of shape {emb.shape}")
    elif m["loader"] == "api":
        ...
        model = SentenceTransformer(m["name"], trust_remote_code=True)
        first_module = model[0]
        if hasattr(first_module, "emb_layer"):  # static embeddings (GloVe / Komninos)
            emb = first_module.emb_layer.weight.detach().cpu().numpy()
            tok = list(first_module.tokenizer.vocab)
        else:
            try:
                emb = model[0].auto_model.get_input_embeddings().weight.detach().cpu().numpy()
            except (NotImplementedError, AttributeError):
                if hasattr(model[0].auto_model, "embeddings") and hasattr(model[0].auto_model.embeddings, "word_embeddings"):
    ...
    print(f"[{key}] PCA ({emb.shape[0]} pts) ...")
    from sklearn.decomposition import PCA
    xy = PCA(n_components=2, random_state=42).fit_transform(emb).astype(np.float32)
...
def make_figure(data, label, ntok):
    ...
    fig.update_layout(
        title=f"{label} — {ntok} tokens (PCA 2D)", template="plotly_white",
...
function runSearch() {{
  ...
  neigh = (typeof D.randomNeighbors === "function") ? D.randomNeighbors(qi) : D.nn[qi];
  setTrace(D.neighborTrace, neigh); const {{xs,ys}} = setTrace(D.queryTrace, [qi]);
  zoomTo(xs.concat(neigh.map(i=>D.x[i])), ys.concat(neigh.map(i=>D.y[i])));
...
def main():
  ...
  <span class="sep">|</span>
  <strong>Seed:</strong>
  <input id="seed" type="number" value="42" style="width:48px;font-size:13px;padding:3px 6px;border:1px solid #bbb;border-radius:4px;">
  <button onclick="regenerateRandom()">Regenerate Vectors</button>
  <span class="sep">|</span>
"""
            extra_js = """
let currentSeed = 42;
...
// Pre-calculate trace mappings at page load to support coordinate restyling
const CATS = ["word", "number", "punctuation", "special", "other"];
...
CATS.forEach(c => {
  ...
  if (indices.length > 0) {
    trace_mappings.push(indices);
  }
});
D.trace_mappings = trace_mappings;

function regenerateRandom() {
  const seedInput = parseInt(document.getElementById("seed").value) || 42;
  currentSeed = seedInput;
  status.textContent = "Regenerating random 2084d vectors (Seed: " + seedInput + ")...";
  setTimeout(() => {
    const rand = seededRandom(seedInput);
    const newX = [], newY = [];
    for (let i = 0; i < D.tok.length; i++) {
      const u1 = rand() || 0.0001, u2 = rand() || 0.0001;
      const z0 = Math.sqrt(-2.0 * Math.log(u1)) * Math.cos(2.0 * Math.PI * u2);
      const z1 = Math.sqrt(-2.0 * Math.log(u1)) * Math.sin(2.0 * Math.PI * u2);
      newX.push(Number((z0 * 3.0).toFixed(3)));
      newY.push(Number((z1 * 3.0).toFixed(3)));
    }
    D.x = newX; D.y = newY;
    D.randomNeighbors = function(qi) {
      const qiRand = seededRandom(qi + seedInput * 1000000);
      const list = [];
      while (list.length < 15) {
        const idx = Math.floor(qiRand() * D.tok.length);
        if (idx !== qi && !list.includes(idx)) {
          list.push(idx);
        }
      }
      return list;
    ...
    // Update Plotly category traces with exact index preservation
    for (let t = 0; t < D.trace_mappings.length; t++) {
      const xs = D.trace_mappings[t].map(i => D.x[i]);
      const ys = D.trace_mappings[t].map(i => D.y[i]);
      Plotly.restyle(gd, { x: [xs], y: [ys] }, [t]);
    }
    
    clearSearch();
    status.textContent = "Regenerated 2084d random baseline (Seed: " + seedInput + ")! Try searching a token.";
  }, 50);
}
"""
        ...
        out = f"html/atlas_{m['key']}.html"
        with open(out, "w") as f:
            f.write(html)
        print(f"wrote {out} ({os.path.getsize(out)//1_000_000} MB)")

    if built_models:
        print(f"\nDone. Open html/atlas_{built_models[0]['key']}.html and use the model dropdown to switch.")
    else:
        print("\nDone. No models successfully built.")
